...
This script automates the processing of radio astronomy data to produce scientifically usable outputs, such as cleaned images or statistical summaries. It demonstrates a complete workflow, including:
...
- The script assumes a dummy Measurement Set (`data/dummy_data.ms`).
- For real data, replace it with an actual Measurement Set and uncomment the `importasdm` task to convert raw telescope data (e.g., ALMA ASDM format) to MS format.

### Run the Script

To execute the script, use the following command in the CASA environment:

```bash
casapy -c casa_data_processing.py
```

Alternatively, you can run it in a Python environment with CASA modules:
...
Outputs are saved in the `outputs/` directory. See the "Outputs" section below for details.
...
1. **Import and Inspect**: Imports a Measurement Set and generates a summary (`obs_summary.txt`). Creates a diagnostic plot (`amp_vs_time.png`) to check for RFI or anomalies.
2. **Flagging**: Flags shadowed data, high-amplitude outliers, and specific bad antennas (e.g., ANT1).
3. **Calibration**: Applies bandpass calibration using a bright calibrator (`CAL_BP`). Performs gain calibration for amplitude and phase (`CAL_PHASE`). Applies calibrations to the science target (`TARGET`).
4. **Imaging**: Creates a cleaned image using the CLEAN algorithm (`target_image`). Exports the image as a FITS file (`target_image.fits`).
5. **Analysis**: Computes image statistics (peak flux, RMS noise) and saves them (`image_stats.txt`).
6. **Cleanup**: Lists all generated files for reference and optionally cleans up temporary calibration tables (commented out).

## Troubleshooting

- **Missing MS File**: If `dummy_data.ms` is not found, replace it with a real Measurement Set or use `importasdm` for raw data.
...
For questions, issues, or contributions, contact Mojtaba Raouf at [mojtaba.raouf@gmail.com](mailto:mojtaba.raouf@gmail.com).

## Acknowledgments

CASA is developed by the National Radio Astronomy Observatory (NRAO). This script is inspired by standard radio astronomy data reduction workflows.
